...
boolean Vmax, Vmin, Vpp, Vave, Freq, Period, RF;
float max, min;
PFont f;
float vScale, tScale;
...
void setup() {
  ...
  //serial link setup
  port = new Serial(this, "COM32", 115200);
  //buff = new int[1200];
  smooth();
} //end setup
...
public void Reset_Trig(int theValue) {
  ...
  cp5.controller("trigger").setValue(512);
}
...
public void Vpp(boolean flag) {
  if (flag == true) {
    Vpp = true;
    Vmax = true;
    cp5.controller("Vmax").setValue(1);
    Vmin = true;
    cp5.controller("Vmin").setValue(1);
  } else {
    Vpp = false;
  }
}
...
void draw() {
  ...
  if ((Tcursor1==true) && (Tcursor2==true)) {
    ...
    text("\u0394 Time = "+dt_1+" ms", 1490, 575);
    text("Freq = "+(freq_1)+" Hz", 1490, 605);
  }
  ...
  if (Vcursor1==true) {
    ...
    text("Voltage 1 = "+v1_1+" V", 1650, 515);
  }
  ...
  for (int j=0; j<(buffLength-windowLength-1); j++) {
  ...
  if (triggered == true) {
    for (int i=windowOffset; i<(windowLength-1+windowOffset); i++){
  ...
  textFont(f, 16);
  fill(0);
  for (int x=windowLength-1; x>0; x--) {
    int x1 = x+25;
    int x2 = x+26;
    int y1 = 900+50-getY(windowBuff[x-1])+(475-Math.round(offset));
    int y2 = 900+50-getY(windowBuff[x])+(475-Math.round(offset));
  ...
  if (Vave == true) {
    ...
    ave = total/windowLength;
    int aveY = totalY/windowLength;
    line(25,aveY,1225,aveY);
    String ave1 = String.format("%.3f", ave);
    text("Vave = "+ave1+" V", 1650, 725);
  }
} //end draw
