...
int windowLength = 11361;
int windowOffset = 0;
boolean triggered = false;
int timeLength = 1200;
float samplingRate = 11.36; //Sampling rate in kHz
...
void setup() {
  ...
  //serial link setup
  port = new Serial(this, "COM21", 1000000);
  smooth();
} //end setup
...
void draw() {
  ...
  int hValI = Math.round(abs(hVal/.088028));
  ...
  for (int j=0; j<(buffLength-timeLength-1); j++) {
    ...
    if (RF == false) {
      if ((trigger_p > trig_val1) && (trigger_p < trig_val2)){
        ...
        triggered = true;
        if (SC == true) {
          cp5.controller("Stop_Run").setValue(1);
        }
        break;
      } else {
        triggered = false;
  ...
  if (triggs == true){
   ...
   freqT = 1/(diff * .000088028); 
   if(abs(freqT - prevFreqT) < 20){
     freqT = prevFreqT;
   }
   period = diff * .000088028;
   prevFreqT = freqT;
